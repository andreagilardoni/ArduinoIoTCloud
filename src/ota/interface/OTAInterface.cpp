...
#endif // !defined OFFLOADED_DOWNLOAD
...
OTACloudProcessInterface::State OTACloudProcessInterface::startOTA() {
  ...
  if(res == HTTP_ERROR_CONNECTION_FAILED) {
    ...
    DEBUG_ERROR("OTA ERROR: http client timeout \"%s\"", context->url);
    return OtaHeaderTimeoutFail;
  } else if(res != HTTP_SUCCESS) {
    DEBUG_ERROR("OTA ERROR: http client returned %d on  get \"%s\"", res, context->url);
