...
public:
     ...
     */
    virtual Status decode(Message* msg, uint8_t* buf, size_t &len) = 0;

    /**
     * sets the preallocated empty message structure that needs to be decoded, before iterativley decoding it
     * @param Message the message that needs to be decoded, msg should be big enough to accomodate all the required fields
     */
