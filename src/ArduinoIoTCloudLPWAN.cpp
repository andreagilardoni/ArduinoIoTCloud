...
ArduinoIoTCloudLPWAN::State ArduinoIoTCloudLPWAN::handle_Connected()
{
  ...
  updateTimestampOnLocallyChangedProperties(getThing().getPropertyContainer());
...
void ArduinoIoTCloudLPWAN::decodePropertiesFromCloud()
{
  ...
  CBORDecoder::decode(getThing().getPropertyContainer(), lora_msg_buf, bytes_received);
}
...
void ArduinoIoTCloudLPWAN::sendPropertiesToCloud()
{
  ...
  if (CBOREncoder::encode(getThing().getPropertyContainer(), data, sizeof(data), bytes_encoded, getThing().getPropertyContainerIndex(), true) == CborNoError)
    if (bytes_encoded > 0)
